...
from ...models.secure import PrivateUser, Token as TokenModel
...
class User(BaseType):
    ...
    def __init__(
        ...
        """
        # Initialize BaseType
        super().__init__(connection, row, Identifier("public", "users"))
    ...
    async def to_private(self) -> PrivateUser:
        ...
        tokens: list[TokenModel] = [
            await token.to_model()
            for token in await self.secure.get_tokens(self.id)
        ]
        return PrivateUser(
            id=self.id,
            created_at=self.created_at,
    ...
    async def check_password(
        ...
        """
        Checks if an input password matches the stored password.
        ...
        """
        # Get password
        row: DictRow = await self.secure.get_password(user_id=self.id)
        return await crypt_context.verify(password, row["hash"])
